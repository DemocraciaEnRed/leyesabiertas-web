...
const AuthorSubscribeButton = styled.div`
  cursor:pointer;
  border: none;
  padding:9px 20px;
  font-size: 1.4rem;
...
class SubscribeButton extends Component {
  ...
  async handleAuthorSubscribeClick () {
    // prevent propagation to avoid triggering the onClick event of the parent element
    const token = this.props.authContext && this.props.authContext.keycloak && this.props.authContext.keycloak.token;
    const authorId = this.props.authorId
  ...
  render () {
    ...
    return (
      <AuthorSubscribeButton onClick={(e) => this.handleAuthorSubscribeClick(e)} active={this.state.isSubscribed}>
